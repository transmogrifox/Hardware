...
\begin{figure}[H]
	\centerline{\includegraphics [
		width=9cm,
		keepaspectratio,
		] {illustrations/peak_current_traces.png}}
	\caption{Peak current controller frequency response, $H_v(z)$, plots with different values of $\alpha$. }
	\label{PCM_mag_resp}
\end{figure}
...
Where the constants are expanded per below.
\begin{align*}
a_{dg} = & -\frac{1}{2T_{SW}} \bigg [ \frac{2m_c + m_d}{m_c^2+2m_cm_d+m_d^2} \bigg ] \nonumber\\
b_{dg} = & \frac{1}{2T_{SW}} \bigg [ \frac{m_d}{m_c^2+2m_cm_d+m_d^2} \bigg ] \nonumber\\
c_{dg} = & \frac{1}{T_{SW}} \bigg [ \frac{m_c}{m_c^2+2m_cm_d+m_d^2} \bigg ]\nonumber\\
d_{dg} = & \frac{m_c^2}{m_c^2+2m_cm_d+m_d^2}\nonumber\\
e_{dg} = & \frac{m_c m_d}{m_c^2+2m_cm_d+m_d^2}\nonumber\\
f_{dg} = & \frac{T_{SW}}{2} \bigg [ \frac{m_c^2 m_d}{m_c^2+2m_cm_d+m_d^2} \bigg ]
\end{align*}
