...
Then substituting $a$, $b$, $c$, $d$ and $e$ from coefficients of (\ref{i_dg_canon}),
 ...
 \begin{align}
 \hat{i}_{dg}[n] = &-\frac{m_c + m_d}{T_{SW}} i_{v\infty} \hat{i}_{v}[n] + m_c^2 \hat{i}_v[n-1] \nonumber \\ 
...
\begin{align}
	\hat{i}_{dg}(t) =&  -\frac{\hat{i}_v(t) - \hat{i}_v (t-T_{SW})} {T_{SW}}
	(m_c + m_d) i_{v\infty} \nonumber \\
...
\begin{equation}
\lim_{T_{SW} \to 0} \hat{i}_{v}(t) = -(m_c + m_d) i_{v\infty} \frac{d \hat{i}_v(t)}{dt}
...
\begin{equation}
	 \hat{I}_{dg}(s) =  - (m_c + m_d) i_{v\infty}\hat{I}_{v}(s)s + ( m_c^2 + m_c m_d ) \hat{I}_{v}(s)
\end{equation}
...
The continuous time RHPZ is now apparent in the form derived to this point.  However, further algebraic manipulation is required to see it in its well-known form.
