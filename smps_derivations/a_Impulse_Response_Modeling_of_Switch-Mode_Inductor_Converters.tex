...
The sampling effect was addressed in a further development to the averaged converter behavior presented by Tan, Middlebrook in [ref]. This document introduces a sampler and approximates its effect in the frequency domain to account for it in the averaged model. The sampling effect as introduced by Tan, Middlebrook when coupled with the averaged switch model completes a unified model with improved accuracy in the magnitude response, having negligible error up to the converter Nyquist rate. 

As demonstrated by Jian Li in [ref], the Tan/Middlebrook unified model still fails to predict the high frequency phase response under all conditions.  A further shortcoming identified by Li of the traditional (and enhanced) averaging models is scalability, e.g., accuracy further diminishes when used to approximate variable frequency controllers.
...
Peak Current Programmed Mode Control is a technique used for controlling inductor currents in a switched-mode converter on a cycle-by-cycle decision-making interval. The control signal sets a threshold to limit inductor peak intake currents during the charging portion of the switching cycle. When the inductor current crosses this threshold, a latch is reset to signal shut-down of the primary switch device. A compensating ramp is often added to the controller current sense signal to induce pre-mature tripping of the comparator -- a technique used to stabilize the peak current control loop.

\begin{figure}[ht!]
	\centerline{\includegraphics [
		width=9cm,
		keepaspectratio,
		] {illustrations/Impulse_Extraction.png}}
	\caption{Extraction of the impulse response is done by evaluating system behavior resulting from the application of a pulse.  The impulse response is obtained by deconvolving the pulse pulse response and the pulse.}
	\label{fig:resampling}
\end{figure}


\section{Extraction of the Impulse Response}
The diagram in Fig. \ref{fig:resampling} steps through the analytical process used to understand some key features in the inductor waveform that can be used to express the system impulse response.
