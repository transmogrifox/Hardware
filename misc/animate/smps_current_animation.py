...
with writer.saving(fig, "slope_comp_demo.mp4", vLength/13):
    for i in range(vLength):
        ...
        tnx, icgx, idgx, csx, ivn, d, Fn, hz = get_cycle(Fsw=Fs, offset=-2.0*Ts, iv1=ivn, ic0=ic0+inoise, Vin= Vin+vnoise, Vout=12.5, Lm=18.0*uH, Nps=10.0/7.3, mcmp=mcmp, ccmdcm=True)
        dn = ad*dn + bd*d
        tn.append(tnx)
        icg.append(icgx)
        idg.append(idgx)
        ...
        inoise = inoiseLevel * np.random.randn()
        tnx, icgx, idgx, csx, ivn, d, Fn, hz = get_cycle(Fsw=Fs, offset=-Ts, iv1=ivn, ic0=ic0+inoise, Vin= Vin+vnoise, Vout=12.5, Lm=18.0*uH, Nps=10.0/7.3, mcmp=mcmp, ccmdcm=True)
        dn = ad*dn + bd*d
        tn.append(tnx)
        icg.append(icgx)
        idg.append(idgx)
        ...
        vnoise = noiseLevel* np.random.randn()
        inoise = inoiseLevel * np.random.randn()
        tnx, icgx, idgx, csx, ivn, d, Fn, hz = get_cycle(Fsw=Fs, offset=0.0, iv1=ivn, ic0=ic0+inoise, Vin= Vin+vnoise, Vout=12.5, Lm=18.0*uH, Nps=10.0/7.3, mcmp=mcmp, ccmdcm=True)
        dn = ad*dn + bd*d
        tn.append(tnx)
        icg.append(icgx)
        idg.append(idgx)
